...
public class EmojiconsPopup extends PopupWindow {
	...
	private int keyBoardHeight = App.getContext().getSharedPreferences(MyPREFERENCES, Context.MODE_PRIVATE).getInt(KEYBOARD_SIZE, (int) (App.getScreenHeight()/2.5) );//40percent
...
//		Store.putInt(StoreConstants.KEYBOARD_SIZE,height);
		App.getContext().getSharedPreferences(MyPREFERENCES, Context.MODE_PRIVATE).
			edit().putInt(KEYBOARD_SIZE, (int) (App.getScreenHeight()/2.5) ).
			apply();

		App.showDebugMessage("KEYBOARD_SIZE "+height);
		setWidth(App.getScreenWidth());
		setHeight(height);
	}
